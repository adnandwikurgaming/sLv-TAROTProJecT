...
@section('content')
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-zinc-100 text-3xl font-light tracking-tight mb-2 drop-shadow-[0_0_8px_rgba(255,26,26,0.2)]">Omen Details</h1>
            <p class="text-zinc-500 text-sm">
                {{ $reading->spread_type === 'single' ? 'Daily Curse' : 'Past, Present, Future' }} 
                <span class="mx-2 text-red-900">•</span> 
                <span class="text-zinc-400">{{ $reading->created_at->format('d M Y, H:i') }}</span>
            </p>
        </div>
        <a href="{{ route('tarot.history') }}" class="text-sm font-medium text-zinc-500 hover:text-horror-bloodLight transition-colors flex items-center gap-2 drop-shadow-[0_0_2px_rgba(255,26,26,0.3)]">
