...
@section('content')
    <div class="max-w-md mx-auto mt-16 relative">
        ...
        <div class="bg-zinc-950/80 backdrop-blur-xl border border-red-900/30 shadow-[0_8px_30px_rgba(138,3,3,0.15)] rounded-3xl p-10">
            <h2 class="text-zinc-100 text-3xl font-light tracking-tight text-center mb-2 drop-shadow-[0_0_8px_rgba(255,26,26,0.2)]">Join the Coven</h2>
            <p class="text-center text-zinc-500 text-sm mb-8 font-medium">Begin your dark mystic journey</p>

            <form method="POST" action="{{ route('register') }}">
                @csrf

                <div class="mb-5">
                    <label class="block text-zinc-300 text-xs font-semibold tracking-wider uppercase mb-2">Mortal Name (Full Name)</label>
                    ...
                    @error('name') <p class="text-horror-bloodLight text-xs mt-2 drop-shadow-[0_0_2px_rgba(255,26,26,0.8)]">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="mb-8">
                    <label class="block text-zinc-300 text-xs font-semibold tracking-wider uppercase mb-2">Seal the Pact (Confirm Password)</label>
            ...
            <p class="text-center text-zinc-500 text-sm mt-8">
                Already cursed?
                <a href="{{ route('login') }}" class="text-horror-bloodLight font-semibold hover:underline decoration-red-900/50 underline-offset-4 drop-shadow-[0_0_5px_rgba(255,26,26,0.3)]">Sign In</a>
            </p>
        </div>
    </div>
@endsection